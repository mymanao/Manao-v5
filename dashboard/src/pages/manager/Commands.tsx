...
import type { Command, CustomCommand, Configuration, Permission } from "@/types/api";
...
export function CommandsPage() {
  ...
  const filtered = builtIn.filter(
    (c) => c.name.en.includes(search.toLowerCase()) || c.name.th.includes(search),
  );
    ...
    <Box>
              ...
              {filtered.map((cmd) => {
                ...
                return (
                  <TableRow key={cmd.name.en} hover>
                    <TableCell>
                      <Switch size="small" checked={enabled} onChange={() => toggleCommand(cmd.name.en, enabled)} />
                    </TableCell>
                    <TableCell>
                      <Stack spacing={0.25}>
                        <Typography variant="body2" fontWeight={600}>{cmd.name.en}</Typography>
                        <Typography variant="caption" color="text.secondary">{cmd.name.th}</Typography>
                      </Stack>
                    </TableCell>
                    <TableCell>
                      <Typography variant="body2" color="text.secondary">{cmd.description.en}</Typography>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Stack direction="row" spacing={0.5}>
                        {(cmd.platforms ?? ["twitch", "kick", "discord"]).map((p) => (
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Stack spacing={2}>
            <TextField label="Name (EN)" size="small" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
